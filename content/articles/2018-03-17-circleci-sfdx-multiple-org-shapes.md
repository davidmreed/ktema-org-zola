...
This article assumes that you've followed [Salesforce Lifecycle and Tooling: CircleCI and Salesforce DX](@/articles/2018-02-02-salesforce-dx-circleci.md) and are using a fairly similar `config.yml`. However, the principles are transferable to other continuous integration environments and build sequences.
...
If we define jobs by copying and pasting the [core SFDX build job](@/articles/2018-02-02-salesforce-dx-circleci.md), our `config.yml` can become unwieldy and difficult to maintain. If there's a lot of setup work that significantly differs between the org definitions, it might be necessary nonetheless. 
