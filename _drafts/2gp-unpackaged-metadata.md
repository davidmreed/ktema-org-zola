...
The critical facet of this code is that it establishes a _runtime_, but not a _compile time_, dependency on the field `Account.Test__c` (which we are not packaging). That's the situation for which `unpackagedMetadata` is designed. When we do
...
By contrast, if we use the `does-not-work` package, we can demonstrate that the `unpackagedMetadata` feature does _not_ allow us to satisfy a compile-time dependency. Here's the Apex test from that package:
